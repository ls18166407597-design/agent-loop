...
pid_start_time() {
    ...
    if [ -d "/proc/$pid" ] 2>/dev/null; then
        # Linux: /proc/pid/stat 第 22 个字段是启动时间（clock ticks）
        stat -c %Y "/proc/$pid" 2>/dev/null || echo 0
    elif ps -p "$pid" >/dev/null 2>&1; then
        # macOS: 用 ps 获取启动时间
        local lstart=$(ps -o lstart= -p "$pid" 2>/dev/null)
        if [ -n "$lstart" ]; then
            if [[ "$OSTYPE" == "darwin"* ]]; then
...
CMD_PATTERN=$(get_agent_pattern)
source "$SCRIPT_DIR/adapters/$WORKER_AGENT.sh"
WORK_PATTERN=$(get_agent_pattern)
...
check_and_kill() {
    ...
    [ "$age" -ge "$TIMEOUT" ] && kill -9 "$pid" 2>/dev/null && log "超时 kill ($pattern, AGE=${age}min)" && return 1
    return 0
}
...
ROUND=$(ls "$TASKS"/round-*-commander.log 2>/dev/null | wc -l | tr -d ' ')
ROUND=$((ROUND + 1))
...
[ -z "$CURRENT_PHASE" ] && log "所有阶段完成" && touch "$TASKS/.done" && exit 0
...
log "Round $ROUND: 主 OC 启动 ($COMMANDER_AGENT, session=${CMD_SESSION_ID:-新建})"

CMD_OUTPUT=$(cd "$PROJECT_DIR" && invoke_agent "$COMMANDER_CMD" "$RLOG" "$CMD_SESSION_ID" 2>&1)
CMD_EXIT=$?

NEW_CMD_SESSION=$(echo "$CMD_OUTPUT" | tail -1 | tr -d '[:space:]')
[ -n "$NEW_CMD_SESSION" ] && echo "$NEW_CMD_SESSION" > "$CMD_SESSION_FILE"
log "主 OC EXIT=$CMD_EXIT session=$NEW_CMD_SESSION"

# ─── 检查计划 ───
if ! grep -q "任务\|目标\|步骤\|执行" "$TASKS/next-plan.md" 2>/dev/null; then
    log "无有效计划，跳过"
    exit 0
fi
...
log "Round $ROUND: 工作 OC 启动 ($WORKER_AGENT)"
cd "$PROJECT_DIR" && invoke_agent "$WORKER_CMD" "$WLOG" "" > /dev/null 2>&1
WORK_EXIT=$?
log "工作 OC EXIT=$WORK_EXIT"

# ─── 阶段完成检查 ───
if [ "$WORK_EXIT" -eq 0 ] && grep -q "全部完成\|阶段完成\|所有目标" "$TASKS/report.md" 2>/dev/null; then
